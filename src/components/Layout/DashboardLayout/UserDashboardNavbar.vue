<template>
    <nav class="bg-white border-b border-neutral-1 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto flex justify-between items-center h-16">
            <!-- Left: Logo -->
            <button v-if="showLogo" class="hidden md:flex cursor-pointer items-center space-x-2"
                @click="router.push('/')">
            ...
            <div class="hidden md:flex xl:space-x-6 lg:space-x-4 space-x-3 items-center">
                ...
                <router-link to="/about"
                    class="2xl:text-lg text-[10px] xl:text-sm  text-neutral-6 hover:text-primary-2">About
                    Us</router-link>
                <div class="relative group ">
                    <button
                        :class="isPropertiesActive ? 'bg-primary-5  text-primary-0 px-3 py-2 rounded' : 'text-neutral-6'"
                        class=" navbar_properties 2xl:text-lg text-[10px] xl:text-sm   hover:text-primary-2 flex items-center">
                        Properties
                        <svg class="w-3 h-3 ml-1 transition-transform group-hover:rotate-180" fill="none"
            ...
            <div class="flex items-center lg:space-x-4 md:space-x-2 space-x-4">
                <button aria-label="Wishlist">
                    <img class="2xl:w-fit xl:w-[20px]  md:w-[12px] w-[16px]" src="@/assets/svg/ash-heart-light.svg"
                        alt="" />
                </button>
                <button aria-label="Cart">
                    <img class="2xl:w-fit xl:w-[20px]  md:w-[12px] w-[16px]" src="@/assets/svg/ash-cart-light.svg"
                        alt="" />
                </button>
        ...
        <transition name="fade">
            <div v-if="isMobileMenuOpen" class="fixed inset-0 z-50 bg-white p-6 overflow-y-auto">
                ...
                <div class="space-y-6 mt-10">
                    <router-link to="/" class="block text-lg  text-neutral-8"
                        @click="isMobileMenuOpen = false">Home</router-link>
                    <router-link to="/about" class="block text-lg  text-neutral-8"
                        @click="isMobileMenuOpen = false">About
                        Us</router-link>
                    ...
                    <div>
                        ...
                        <div v-if="isMobilePropertiesOpen" class="mt-2 ml-4 space-y-3">
                            <router-link to="/properties/rent" class="block text-neutral-6"
                                @click="isMobileMenuOpen = false">Rent a
                                House</router-link>
                            <router-link to="/properties/buy" class="block text-neutral-6"
                                @click="isMobileMenuOpen = false">Buy a
                                House</router-link>
                    ...
                    <router-link to="/contact" class="block text-lg  text-neutral-8"
                        @click="isMobileMenuOpen = false">Contact
                        Us</router-link>
                </div>
